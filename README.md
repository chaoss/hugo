...
Some content is mounted from external repositories. To include the Practitioner Guides proof-of-concept content, initialize submodules before building:

```shell
git submodule update --init --depth 1 --filter=blob:none external/wg-data-science
git -C external/wg-data-science sparse-checkout set practitioner-guides
```
